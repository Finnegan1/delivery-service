...
class ComponentIdFilter(BaseFilter):
  def __init__(
    self, 
    name,
    description,
  ...
  def filter(
    ...
    class ComponentID(pydantic.BaseModel):
      ...
      version: typing.Optional[str] = pydantic.Field(None, description='Version of the component')
    class ComponentIDList(pydantic.BaseModel):
      ids: list[ComponentID] = pydantic.Field([], description='A list of ID objects to filter the components.')
    ...
    for id in component_id_list.ids:
      if id.version == None:
...
class ComponentPackageFilter(BaseFilter):
  def __init__(
    self, 
    name: str,
    description: str,
  ...
  def filter(
    ...
    if not package_mentioned_return.specific_packages_mentioned:
      if isinstance(package_mentioned_return.data, Instruction):
        packages = ai.filter_operations.combine_lists(self.pipeline_manager.get_pipeline('package').run(question=package_mentioned_return.data.instruction))
      else:
    ...
    findings = self.db_session.query(
      ...
      deliverydb.model.ArtefactMetaData.type.__eq__(dso.model.Datatype.STRUCTURE_INFO),
      sqlalchemy.or_(deliverydb.util.ArtefactMetadataQueries.component_queries(
        components=[component.identity() for component in self.landscape_components],
      )),
      sqlalchemy.or_(
        *[
          sqlalchemy.and_(
            deliverydb.model.ArtefactMetaData.data.op('->>')('package_name') == package.name,
            deliverydb.model.ArtefactMetaData.data.op('->>')('package_version') == package.version,
          )
      ...
      deliverydb.model.ArtefactMetaData.component_version,
        ...
        sqlalchemy.func.count(sqlalchemy.func.distinct(deliverydb.model.ArtefactMetaData.data.op('->>')('package_name'))) == len(packages)
    ).all()
    
    return set([gci.componentmodel.ComponentIdentity(name=name, version=version) for name, version in findings])
  
class ComponentVulnerabilityFilter(BaseFilter):
  def __init__(
    self, 
    name: str,
    description: str,
  ...
  def filter(
    ...
    findings = self.db_session.query(
      ...
      deliverydb.model.ArtefactMetaData.data.op('->>')('cve').in_([vul.cve for vul in vulnerabilities_mentioned_return.data]),
    ).group_by(
      deliverydb.model.ArtefactMetaData.component_name,
      deliverydb.model.ArtefactMetaData.component_version,
    ).all()
...
# TODO: Testing
class ComponentMalwareFilter(BaseFilter):
  def __init__(
    self, 
    name: str,
    description: str,
  ...
  def filter(
    ...
    for finding in findings:
      ...
      for item in data['findings']:
          if item['name'] in [malware.name for malware in malware_mentioned_return.data]:
            items_with_findings.append(gci.componentmodel.ComponentIdentity(
              name=finding.name, 
...
class ComponentResourcesFilter(BaseFilter):
  def __init__(
    self, 
    name: str,
    description: str,
  ...
  def filter(
    ...
    class ResourceInformation(pydantic.BaseModel):
      ...
      version: typing.Optional[str] = pydantic.Field(description='Version of the resource.')
    ...
    for resource_information in resource_mentioned_return.data:
      new_components_with_resource = [
        ...
        if resource_information.name in [
            resource.name
            for resource
...
class PackageIdFilter(BaseFilter):
  def __init__(
    self, 
    name: str,
    description: str,
  ...
  def filter(
    ...
    class PackageInformation(pydantic.BaseModel):
      ...
      version: typing.Optional[str] = pydantic.Field(description='Version of the package.')
    ...
    return set([ai.pipelines.PackageID(name=name, version=version) for name, version in findings])

class PackageComponentFilter(BaseFilter):
  ...
  def filter(
    ...
    class ComponentInformation(pydantic.BaseModel):
      name: str = pydantic.Field(description='Name of the Component.')
      version: typing.Optional[str] = pydantic.Field(description='Version of the Component.')
...
class PackageVulnerabilityFilter(BaseFilter):
  ...
  def filter(
    ...
    findings = self.db_session.query(
      ...
      deliverydb.model.ArtefactMetaData.data.op('->>')('cve').in_([entry.cve for entry in components_mentioned_return.data])
    ).all()
    
    return set([ai.pipelines.PackageID(name=name, version=version) for name, version in findings])

  
class PackageLicenseFilter(BaseFilter):
  ...
  def filter(
    ...
    license_subquery = self.db_session.query(
    ...
    findings = self.db_session.query(
      ...
      license_subquery.c.license.op('->>')('name').in_([entity.license for entity in licenses_mentioned_return.data])
    ).all()
    
    return set([ai.pipelines.PackageID(name=name, version=version) for name, version in findings])
  
  
class PackageDataSourceFilter(BaseFilter):
  ...
  def filter(
    ...
    findings = self.db_session.query(
      ...
      deliverydb.model.ArtefactMetaData.datasource.in_([data_source.name for data_source in data_sources_mentioned_return.data])
    ).all()
    
    return set([ai.pipelines.PackageID(name=name, version=version) for name, version in findings])
  

class PackageResourceFilter(BaseFilter):
  ...
  def filter(
    ...
    class ResourceInformation(pydantic.BaseModel):
      ...
      version: typing.Optional[str] = pydantic.Field(description='Specific Version of the Resource')
...
class ResourceIDFilter(BaseFilter):
  ...
  def filter(
    ...
    resources_mentioned_return=self.client.chat.completions.create(
      model=os.getenv('OPEN_AI_MODEL'),
      temperature=0.0,
      response_model=SpecificResourcesMentionedReturn,
      messages=[
    ...
    for resource_information in resources_mentioned_return.resource_names:
      new_components_with_resource = [
        component
        for component
        in self.landscape_components
    ...
    return set([ai.pipelines.ResourceID(name=id.name, version=id.version) for id in components_with_resource])