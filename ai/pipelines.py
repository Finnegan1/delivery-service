...
class BasePypeline(abc.ABC):
  ...
  def run(
    self, 
    question: str
  ) -> dict[str, str | list[gci.componentmodel.ComponentIdentity]] | dict[str, str | list[PackageID]]:
    pass
...
class PipelineManager():
  ...
  def decide_which_pipeline(self, question)->BasePypeline:
    ...
    pipelne_decision = ai.new.client.chat.completions.create(
      model=ai.new.OPEN_AI_MODEL,
      temperature=0.0,
